...
const retryTask = (id: number) => {
  setTasks(tasks.map(task =>
    task.id === id
      ? { ...task, status: "Doing", isRetry: true }
      : task
  ))
}
        ...
        <div className="card w-96 bg-base-100 shadow-sm">
          <div className="card-body p-0 h-[500px] flex flex-col">
            <div className="bg-purple-500 rounded-md px-4 py-2 relative">
              ...
              <button className="btn absolute right-0 top-1/2 -translate-y-1/2 bg-transparent border-none shadow-none text-white" 
              onClick={() => {
                setModalStatus("Done")
                openModal()
              }}>
              ...
              {tasks
                ...
                .map(task => (
                  <div key={task.id} className="card bg-base-100 shadow mb-2">
                    <div className="card-body p-3 flex-row justify-between">
                      <div className="">
                      {task.title}
                      </div>
